...
import { getPendingPasses, approvePass } from '../../services/academic.service';
    ...
    <div className="min-h-screen font-sans text-slate-200 relative selection:bg-indigo-500/30">
      ...
      <main className="container mx-auto px-4 py-24 md:py-32 relative z-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-10">
          <div>
            <Link to="/academic/dashboard" className="inline-flex items-center text-sm text-slate-500 hover:text-indigo-400 mb-2 transition-colors">
              <ArrowLeft size={16} className="mr-1" /> Back to Dashboard
            </Link>
            <h1 className="text-3xl md:text-4xl font-bold text-white tracking-tight">Pending Approvals</h1>
            <p className="text-slate-400 mt-1">
              Validating outstation requests from all departments.
            </p>
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-24 bg-slate-900/20 border border-white/5 rounded-2xl border-dashed">
            ...
            <p className="text-slate-500 mt-2 text-center max-w-sm">
              {searchTerm 
                ...
                : 'Great job! You have cleared all pending academic approvals.'}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredPasses.map((pass) => (
              <div 
                key={pass._id} 
                className="group relative bg-slate-900/40 backdrop-blur-md border border-white/10 rounded-2xl overflow-hidden hover:border-indigo-500/30 hover:shadow-xl hover:shadow-indigo-500/10 transition-all duration-300"
              >
                {/* Top Border Gradient */}
                ...
                <div className="p-6 flex flex-col h-full">
                  ...
                  <div className="flex justify-between items-start mb-5">
                    <div className="flex gap-4">
                      <div className="w-12 h-12 rounded-full bg-gradient-to-br from-blue-600 to-indigo-700 flex items-center justify-center text-white font-bold text-lg shadow-inner">
                  ...
                  <div className="flex gap-3 mt-auto pt-4 border-t border-white/5">
                    ...
                    <button
                      ...
                      className="flex-1 py-2.5 px-4 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-sm shadow-lg shadow-indigo-900/20 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {processing === pass._id ? (
