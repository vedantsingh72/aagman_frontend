    ...
    return (
      <div 
        className={`relative group transition-all duration-300 rounded-xl border ${
          focusedField === name ? 'border-lime-500 bg-slate-800/80 shadow-[0_0_0_4px_rgba(132,204,22,0.1)]' : 'border-slate-700 bg-slate-800/40 hover:bg-slate-800/60'
        }`}
      >
        <div className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors duration-300 ${focusedField === name ? 'text-lime-400' : 'text-slate-500'}`}>
          <Icon size={18} />
        </div>
        <input
          type={type}
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center p-4 sm:p-6 relative overflow-hidden bg-[#0A0F1E] font-sans">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-lime-900/20 via-[#0A0F1E] to-[#0A0F1E]"></div>
        <div className="absolute top-[10%] right-[10%] w-[600px] h-[600px] bg-emerald-600/10 rounded-full blur-[120px] animate-pulse-slow"></div>
        <div className="absolute bottom-[-10%] left-[-10%] w-[600px] h-[600px] bg-green-600/10 rounded-full blur-[120px] animate-pulse-slow delay-1000"></div>
      ...
      <div className="relative z-10 w-full max-w-6xl bg-slate-900/40 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-[0_0_50px_0_rgba(0,0,0,0.3)] flex overflow-hidden min-h-[700px]">
        ...
        <div className="w-full lg:w-3/5 p-8 md:p-10 flex flex-col justify-center relative bg-slate-900/30">
          ...
          <div className="mb-8">
            <h2 className="text-3xl font-bold text-white mb-2 flex items-center gap-3">
              Academic Office Registration 
              <span className="hidden sm:inline-flex text-xs font-normal px-2 py-1 rounded-full bg-lime-500/20 text-lime-300 border border-lime-500/30">
                Aagman Portal
              </span>
            </h2>
            <p className="text-slate-400 text-sm">Register for administrative oversight and final approvals.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <InputField 
                name="email" 
                type="email"
                placeholder="academic@college.edu" 
                icon={Mail} 
            ...
            {/* 3. Academic ID & Designation Row */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-slate-300 ml-1 uppercase tracking-wider">Academic ID</label>
                <InputField 
                  name="academicId" 
                  placeholder="ACAD001" 
                  icon={Hash} 
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-slate-300 ml-1 uppercase tracking-wider">Designation (Optional)</label>
                <InputField 
                  name="designation" 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <div className={`relative group transition-all duration-300 rounded-xl border ${focusedField === 'password' ? 'border-lime-500 bg-slate-800/80' : 'border-slate-700 bg-slate-800/40 hover:bg-slate-800/60'}`}>
                  <div className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors duration-300 ${focusedField === 'password' ? 'text-lime-400' : 'text-slate-500'}`}>
                    <Lock size={18} />
                  </div>
                  <input
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-lime-600 to-emerald-600 hover:from-lime-500 hover:to-emerald-500 text-white font-bold py-4 rounded-xl shadow-lg shadow-lime-500/20 transition-all duration-300 transform hover:-translate-y-0.5 active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2 mt-6"
            >
              {loading ? (
                <>
          ...
          <div className="mt-8 text-center border-t border-white/5 pt-6">
            <p className="text-slate-500 text-sm">
              ...
              <Link to="/login" className="text-white hover:text-lime-400 font-medium transition-colors">
                Log In
              </Link>
            </p>
          </div>
        ...
        <div className="hidden lg:flex w-2/5 bg-gradient-to-bl from-lime-900/40 to-slate-900/40 relative items-center justify-center p-12 overflow-hidden border-l border-white/5">
          ...
          <div className="relative z-10 flex flex-col items-center text-center">
            <div className="relative mb-8 group">
              <div className="absolute inset-0 bg-lime-500/30 rounded-full blur-2xl group-hover:bg-lime-500/50 transition-all duration-500"></div>
              <div className="relative w-32 h-32 bg-slate-900/50 border border-white/10 backdrop-blur-md rounded-3xl flex items-center justify-center shadow-2xl transform rotate-6 group-hover:rotate-0 transition-transform duration-500">
                <BookOpenCheck className="text-lime-400 w-16 h-16 drop-shadow-[0_0_15px_rgba(132,204,22,0.5)]" />
              </div>
            ...
            <div className="space-y-4 text-left w-full max-w-xs">
              <div className="flex items-center gap-4 bg-white/5 border border-white/5 p-3 rounded-xl hover:bg-white/10 transition-colors cursor-default">
                ...
                <div>
                  <p className="text-slate-200 text-sm font-bold">Final Approvals</p>
                  <p className="text-slate-500 text-xs">Ultimate decision authority</p>
                </div>
              </div>
              <div className="flex items-center gap-4 bg-white/5 border border-white/5 p-3 rounded-xl hover:bg-white/10 transition-colors cursor-default">
                ...
                <div>
                  <p className="text-slate-200 text-sm font-bold">System Oversight</p>
                  <p className="text-slate-500 text-xs">Monitor all activities</p>
                </div>
              </div>
              <div className="flex items-center gap-4 bg-white/5 border border-white/5 p-3 rounded-xl hover:bg-white/10 transition-colors cursor-default">
                ...
                <div>
                  <p className="text-slate-200 text-sm font-bold">Centralized Control</p>
                  <p className="text-slate-500 text-xs">Unified management</p>
                </div>
              </div>
            </div>
          </div>
